...
are different from the other file and were not explained. The two files were merged by a common traffic measure: `7-day average traffic count`(ADT) along with the coordinates and the road name. We transfer the coordinates from NZTM to latitudes and longitudes since they are easier to work with python mapping packages (e.g. folium or mapbox). Details on data cleaning and merging can be found at [1_data_cleaning.ipynb](1_data_cleaning.ipynb). 

## Exploratory analysis
In this analysis we use ADT as the representitive measure of traffic volume since ADT is one of the common traffic count measure in both files. We note that from the initial exploratory analysis of file `traffic-counts-to-march-2018-2019.csv`, we found all other traffic volume measures are highly correlated with ADT, so ADT is likely to be a good representative measure for traffic volume.
...
In particular, we notice that the sampling density increased noticeably since 2010. 
The traffic volume reached 
a peak around 2012, followed by a clear decrease, both in the high percentile traffic volumes and in the number of traffic counts. 
...
more comparible to each other. The following figures presents a closer view at the data after 2010.
